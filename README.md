...
*«Nastavnik»* - рекомендационная и аналитическая система для непрерывного повышения профессионального мастерства. Базируется на цифровом следе. Система опробовалась на нескольких пилотных школах и с профессиональным сообществом: формировалась система метрик, дополнялся функционал. Техническая реализация включала: 
* предобрабботку данных (входные данные: `xlsx`, `csv`, `pdf`, парсинг с веб-страниц). Конвертация `pdf` в `xlsx` c помощью `tabula` на [gitflic](https://gitflic.ru/project/petitmx/pdf_to_excel). Парсинг данных с вебсайта о курсах с помощью `selenium` и `BeautifulSoup`, пример на [gitflic](https://gitflic.ru/project/petitmx/dpomos_course_parsing).  
